...
class StockAnalyzer:
    ...
    def load_company_index(self, ticker: str):
        """
        CRITICAL FIX: Load an existing company's index from Supabase
        This is called when user selects a company from the dropdown
        """
        ...
        try:
            ...
            # Load index from vector store (filtering by ticker metadata)
            self.index = VectorStoreIndex.from_vector_store(
                self.vector_store,
                storage_context=storage_context
            )
            
            # Create query engine with citation prompt
